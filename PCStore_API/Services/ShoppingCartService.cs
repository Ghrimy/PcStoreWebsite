...
public class ShoppingCartService(PcStoreDbContext context, ILogger<ShoppingCartService> logger) : IShoppingCartService
{
    ...
    public async Task<ShoppingCartDto> AddToShoppingCartAsync(int userId, ShoppingCartAddDto item)
    {
        ...
        var product = await context.Products.FindAsync(item.ProductId) ?? throw new ArgumentException($"Product {item.ProductId} not found.");;
        ...
        if (existingItemInCart == null)
        {
            ...
            if(item.Quantity > product.ProductStock) throw new ArgumentException($"Product {item.ProductId} has only {product.ProductStock} in stock.");
        ...
        else
        {
            ...
            if (existingItemInCart.Quantity + item.Quantity > product.ProductStock) throw new ArgumentException("Quantity exceeds available stock.");
    ...
    public async Task<ShoppingCartDto> RemoveFromShoppingCartAsync(int userId, List<ShoppingCartRemoveDto> items)
    {
        ...
        if(items.Count == 0) throw new ArgumentException("No items to remove");
        ...
        foreach (var product in items)
        {
            ...
            if (existingItem == null)
                throw new ArgumentException($"Product {product.ProductId} not found.");
    ...
    public async Task<ShoppingCartDto> UpdateShoppingCartAsync(int userId, List<ShoppingCartUpdateDto> items)
    {
        ...
        if(items.Count == 0) throw new ArgumentException("No items to update");
        ...
        foreach (var product in items)
        {
            ...
            if (product.NewQuantity <= 0)
                throw new ArgumentException($"Product {product.ProductId} with Quantity: {product.NewQuantity}  can't be/less zero.");
            
            if (product.NewQuantity > existingItem.Product.ProductStock)
                throw new ArgumentException($"Product {product.ProductId} with Quantity: {product.NewQuantity}  can't be more than stock.");
